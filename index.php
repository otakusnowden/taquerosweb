...
// ── Bootstrap backend (session, autoload, .env) ───────────────────────────
require_once __DIR__ . '/config/app.php';

use App\Core\Auth;

$authUser = Auth::user();  // null si no hay sesión activa
...
// Precio mínimo / máximo para AggregateOffer Schema.org
...
$features = [
    ...
    'pro' => [
        'inc' => [
            ...
            'E-commerce completo (hasta 100 productos)',
            'Panel de administración',
            'SEO profesional + Google My Business',
            'Blog avanzado con CMS',
            'Dominio .mx por 1 año incluido',
    ...
    'premium' => [
        'inc' => [
            ...
            'Plataformas y apps web',
            'SEO avanzado y estrategia digital',
...
.navbar-logo { display: flex; align-items: center; gap: 0.5rem; font-family: var(--font-display); font-size: 1.8rem; letter-spacing: 1px; }
...
.nav-links a.nav-login { color: var(--color-orange); font-weight: 700; }
.nav-links a.nav-login:hover { color: #ff8c5a; }
...
/* Dashboard link in nav (when logged in) */
.btn-nav-ghost {
...
.mobile-menu a.nav-login { color: var(--color-orange); }
...
.section-title { font-family: var(--font-display); font-size: clamp(2.2rem, 5vw, 3.5rem); line-height: 1.05; letter-spacing: 1px; margin-bottom: 1rem; }
...
.hero::before {
  ...
  background-image: linear-gradient(rgba(255,107,43,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(255,107,43,0.04) 1px, transparent 1px);
...
.hero::after {
  ...
  background: radial-gradient(circle, rgba(255,107,43,0.12) 0%, transparent 70%); pointer-events: none;
}
...
.hero-eyebrow {
  ...
  border-radius: 100px; padding: 0.4rem 1rem;
  font-size: 0.8rem; font-weight: 600; color: var(--color-gold); margin-bottom: 1.5rem;
  animation: fadeInUp 0.6s ease forwards;
}
.hero-title { font-family: var(--font-display); font-size: clamp(3rem, 7vw, 5.5rem); line-height: 1; letter-spacing: 2px; margin-bottom: 1.5rem; animation: fadeInUp 0.7s ease 0.1s both; }
.hero-title .line-orange { display: block; background: var(--grad-hero); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
.hero-subtitle { font-size: 1.125rem; color: var(--color-muted); line-height: 1.7; margin-bottom: 2.5rem; max-width: 480px; animation: fadeInUp 0.7s ease 0.2s both; }
.hero-actions { display: flex; gap: 1rem; flex-wrap: wrap; margin-bottom: 3rem; animation: fadeInUp 0.7s ease 0.3s both; }
.btn-primary { display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.9rem 2rem; border-radius: var(--radius-btn); font-size: 1rem; font-weight: 700; background: var(--grad-hero); color: #fff; transition: var(--transition); position: relative; overflow: hidden; }
...
.btn-secondary { display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.9rem 2rem; border-radius: var(--radius-btn); font-size: 1rem; font-weight: 700; background: rgba(255,255,255,0.05); color: var(--color-text); border: 1px solid var(--color-border); transition: var(--transition); }
...
.browser-mock { background: #1A1714; border-radius: 6px; border: 1px solid rgba(255,255,255,0.1); overflow: hidden; box-shadow: 0 32px 80px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.05); }
.browser-mock img { display: block; width: 100%; height: auto; object-fit: cover; }
.float-badge { position: absolute; background: var(--color-card); border: 1px solid var(--color-border); border-radius: 6px; padding: 0.75rem 1rem; font-size: 0.8rem; font-weight: 600; display: flex; align-items: center; gap: 0.5rem; box-shadow: 0 16px 40px rgba(0,0,0,0.5); animation: float 3s ease-in-out infinite; z-index: 10; }
...
.hero-banner-mobile { display: none; margin: 2rem 0 1rem; border-radius: 6px; overflow: hidden; box-shadow: 0 16px 48px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,107,43,0.15); position: relative; }
.hero-banner-mobile img { display: block; width: 100%; height: auto; object-fit: cover; }
.hero-banner-mobile-label { position: absolute; bottom: 0.75rem; left: 0.75rem; background: rgba(11,10,8,0.82); backdrop-filter: blur(8px); border: 1px solid rgba(255,107,43,0.3); border-radius: 4px; padding: 0.4rem 0.75rem; font-size: 0.72rem; font-weight: 700; color: var(--color-orange); display: flex; align-items: center; gap: 0.4rem; }

/* ===== TICKER ===== */
.ticker { overflow: hidden; border-top: 1px solid var(--color-border); border-bottom: 1px solid var(--color-border); background: rgba(255,107,43,0.04); padding: 1rem 0; }
.ticker-inner { display: flex; gap: 3rem; animation: marquee 25s linear infinite; white-space: nowrap; }
...
.pricing-card { background: var(--grad-card); border: 1px solid var(--color-border); border-radius: var(--radius-card); padding: 2rem; position: relative; transition: var(--transition); cursor: pointer; overflow: hidden; }
.pricing-card::before { content: ''; position: absolute; inset: 0; background: radial-gradient(circle at 50% 0%, rgba(255,107,43,0.08), transparent 70%); opacity: 0; transition: var(--transition); }
...
.card-badge { display: inline-flex; align-items: center; gap: 0.4rem; padding: 0.35rem 0.85rem; border-radius: 100px; font-size: 0.7rem; font-weight: 800; letter-spacing: 0.06em; text-transform: uppercase; margin-bottom: 1.25rem; position: relative; z-index: 1; }
...
.btn-demo { width: 100%; padding: 0.75rem; border-radius: var(--radius-btn); font-size: 0.82rem; font-weight: 700; background: transparent; color: var(--color-muted); border: 1px dashed rgba(255,255,255,0.12); transition: var(--transition); display: flex; align-items: center; justify-content: center; gap: 0.5rem; cursor: pointer; position: relative; overflow: hidden; }
...
/* ===== DEMO MODAL ===== */
.demo-modal-overlay { position: fixed; inset: 0; z-index: 3000; background: rgba(0,0,0,0.92); backdrop-filter: blur(16px); display: flex; align-items: center; justify-content: center; opacity: 0; pointer-events: none; transition: opacity 0.35s cubic-bezier(0.4,0,0.2,1); }
.demo-modal-overlay.open { opacity: 1; pointer-events: all; }
.demo-modal { width: 94vw; max-width: 1100px; height: 85vh; max-height: 750px; background: #111; border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; overflow: hidden; display: flex; flex-direction: column; transform: scale(0.92) translateY(20px); transition: transform 0.4s cubic-bezier(0.34,1.56,0.64,1), opacity 0.35s ease; opacity: 0; box-shadow: 0 48px 120px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.05); }
.demo-modal-overlay.open .demo-modal { transform: scale(1) translateY(0); opacity: 1; }
.demo-modal-header { display: flex; align-items: center; justify-content: space-between; padding: 1rem 1.5rem; border-bottom: 1px solid rgba(255,255,255,0.08); background: rgba(26,23,20,0.95); flex-shrink: 0; }
.demo-modal-info { display: flex; align-items: center; gap: 1rem; }
.demo-modal-badge { display: inline-flex; align-items: center; gap: 0.4rem; background: rgba(139,218,79,0.12); border: 1px solid rgba(139,218,79,0.25); border-radius: 100px; padding: 0.3rem 0.8rem; font-size: 0.68rem; font-weight: 700; color: var(--color-lime); text-transform: uppercase; letter-spacing: 0.06em; }
...
.btn-fullscreen { display: inline-flex; align-items: center; gap: 0.4rem; padding: 0.5rem 1rem; border-radius: 4px; font-size: 0.78rem; font-weight: 600; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); color: var(--color-text); transition: var(--transition); }
.btn-fullscreen:hover { background: rgba(255,255,255,0.12); border-color: rgba(255,255,255,0.2); }
.btn-demo-close { width: 36px; height: 36px; border-radius: 4px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); color: var(--color-text); font-size: 1.1rem; display: flex; align-items: center; justify-content: center; transition: var(--transition); }
...
.demo-thumb { flex-shrink: 0; padding: 0.5rem 1rem; border-radius: 4px; font-size: 0.75rem; font-weight: 600; color: var(--color-muted); background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.06); cursor: pointer; transition: var(--transition); white-space: nowrap; display: flex; align-items: center; gap: 0.4rem; }
...
.btn-buy-modal { flex-shrink: 0; margin-left: auto; padding: 0.5rem 1.25rem; border-radius: 4px; font-size: 0.78rem; font-weight: 700; background: var(--grad-hero); color: #fff; transition: var(--transition); }
...
.service-card { background: var(--color-card); border: 1px solid var(--color-border); border-radius: 6px; padding: 1.75rem; transition: var(--transition); }
...
.proceso-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-top: 3.5rem; position: relative; }
.proceso-grid::before { content: ''; position: absolute; top: 2.5rem; left: 12.5%; width: 75%; height: 2px; background: linear-gradient(90deg, var(--color-orange), var(--color-red), var(--color-gold)); opacity: 0.3; }
.step-card { background: var(--color-card); border: 1px solid var(--color-border); border-radius: 6px; padding: 2rem 1.5rem; text-align: center; transition: var(--transition); position: relative; }
...
.beneficio-icon-wrap { font-size: 1.5rem; width: 48px; height: 48px; background: rgba(255,107,43,0.1); border: 1px solid rgba(255,107,43,0.2); border-radius: 6px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
.av-1{background:linear-gradient(135deg,#FF6B2B,#E8294C)} .av-2{background:linear-gradient(135deg,#E8294C,#7B1FA2)} .av-3{background:linear-gradient(135deg,#FFB800,#FF6B2B)} .av-4{background:linear-gradient(135deg,#8BDA4F,#FFB800)} .av-5{background:linear-gradient(135deg,#7B1FA2,#E8294C)} .av-6{background:linear-gradient(135deg,#FF6B2B,#FFB800)}
...
/* Alert inside form */
.form-alert { padding: 0.85rem 1.1rem; border-radius: 4px; font-size: 0.83rem; margin-bottom: 1.25rem; line-height: 1.5; display: none; }
.form-alert-error { background: rgba(232,41,76,.1); border: 1px solid rgba(232,41,76,.25); color: #f87093; }
.form-alert-success { background: rgba(139,218,79,.1); border: 1px solid rgba(139,218,79,.25); color: var(--color-lime); }

.package-selector { display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.5rem; margin-bottom: 1.25rem; }
.pkg-option { padding: 0.6rem 0.5rem; border-radius: 4px; font-size: 0.75rem; font-weight: 700; color: var(--color-muted); background: rgba(255,255,255,0.04); border: 1px solid var(--color-border); cursor: pointer; text-align: center; transition: var(--transition); }
...
.form-input, .form-textarea, .form-select {
  ...
  border-radius: 4px; color: var(--color-text); font-size: 0.9rem; font-family: var(--font-body);
...
.form-input:focus, .form-textarea:focus, .form-select:focus { border-color: rgba(255,107,43,0.5); background: rgba(255,107,43,0.04); box-shadow: 0 0 0 3px rgba(255,107,43,0.1); }
.form-input.input-error, .form-textarea.input-error, .form-select.input-error { border-color: var(--color-red) !important; box-shadow: 0 0 0 3px rgba(232,41,76,0.15) !important; }
.form-textarea { resize: vertical; min-height: 100px; }
.form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; }
.field-error { font-size: 0.72rem; color: #f87093; margin-top: 0.25rem; min-height: 1em; }
.selected-pkg-display { background: rgba(255,107,43,0.08); border: 1px solid rgba(255,107,43,0.2); border-radius: 6px; padding: 1rem; margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center; }
...
.pm-badge { background: rgba(255,255,255,0.05); border: 1px solid var(--color-border); border-radius: 4px; padding: 0.4rem 0.75rem; font-size: 0.72rem; font-weight: 600; color: var(--color-muted); }
...
.btn-buy:disabled { opacity: 0.6; cursor: not-allowed; transform: none; box-shadow: none; }
...
.login-hint { text-align: center; margin-top: 1rem; font-size: 0.82rem; color: var(--color-muted); }
.login-hint a { color: var(--color-orange); font-weight: 700; }
.login-hint a:hover { color: #ff8c5a; }

/* Spinner */
@keyframes spin { to { transform: rotate(360deg); } }
.spinner { display: inline-block; width: 16px; height: 16px; border: 2px solid rgba(255,255,255,0.3); border-top-color: #fff; border-radius: 50%; animation: spin 0.6s linear infinite; vertical-align: middle; margin-right: 0.35rem; }
...
.cta-banner-inner { background: var(--color-card); border: 1px solid rgba(255,107,43,0.2); border-radius: 8px; padding: 4rem 3rem; text-align: center; position: relative; overflow: hidden; }
...
.social-btn { width: 38px; height: 38px; border-radius: 4px; background: rgba(255,255,255,0.06); border: 1px solid var(--color-border); display: flex; align-items: center; justify-content: center; color: var(--color-muted); text-decoration: none; transition: var(--transition); }
...
.social-btn.sb-fb:hover { background:#1877F2; border-color:#1877F2; color:#fff; }
.social-btn.sb-ig:hover { background:#E1306C; border-color:#E1306C; color:#fff; }
.social-btn.sb-tk:hover { background:#000; border-color:#000; color:#fff; }
.social-btn.sb-in:hover { background:#0A66C2; border-color:#0A66C2; color:#fff; }
.social-btn.sb-wa:hover { background:#25D366; border-color:#25D366; color:#fff; }
...
/* ===== WHATSAPP FLOAT ===== */
.whatsapp-float { position: fixed; bottom: 2rem; right: 2rem; z-index: 500; width: 56px; height: 56px; border-radius: 50%; background: #25D366; display: flex; align-items: center; justify-content: center; box-shadow: 0 8px 24px rgba(37,211,102,0.4); transition: var(--transition); animation: pulse-wa 2s ease-in-out infinite; text-decoration: none; }
...
<nav class="navbar" id="navbar">
  ...
  <ul class="nav-links">
    ...
    <?php if ($authUser): ?>
      <li><a href="/dashboard" class="nav-login">👤 Mi panel</a></li>
    <?php else: ?>
      <li><a href="/login" class="nav-login">🔑 Iniciar sesión</a></li>
    <?php endif; ?>
  </ul>
  <div class="nav-cta">
    <?php if ($authUser): ?>
...
<div class="mobile-menu" id="mobileMenu">
  ...
  <?php if ($authUser): ?>
    <a href="/dashboard" class="mobile-link nav-login">👤 Mi panel</a>
  <?php else: ?>
    ...
    <div class="hero-inner">
      <div class="hero-content">
        ...
        <div class="hero-banner-mobile">
          <img src="images/taquerosweb_banner.jpg" alt="TaquerosWeb — Sitios web profesionales para negocios mexicanos" width="800" height="450" loading="lazy"/>
          <div class="hero-banner-mobile-label"><span>🚀</span> Sitio en producción</div>
        </div>
      </div>
      ...
      <div class="hero-visual">
        ...
        <div class="mockup-wrap">
          <div class="float-badge badge-1"><span class="badge-dot"></span> Sitio en producción 🚀</div>
          <div class="browser-mock">
            <img src="images/taquerosweb_banner.jpg" alt="Ejemplo de sitio web para taquería hecho por TaquerosWeb" width="600" height="338" loading="eager"/>
          </div>
          <div class="float-badge badge-2">🌮 +320 sitios entregados</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="container">
    ...
    <div class="pricing-grid" id="pricingGrid">
      <?php foreach ($paquetes as $key => $p):
        $f         = $features[$key] ?? ['inc' => [], 'exc' => []];
        $cardClass = 'pricing-card reveal';
        if ($p['tipo_badge'] === 'hot')  $cardClass .= ' featured';
        if ($p['tipo_badge'] === 'best') $cardClass .= ' best';
        $pkgLabel  = htmlspecialchars($p['nombre'] . ' — ' . $p['subtitulo'], ENT_QUOTES);
        $dataPrice = ($p['precio_desde'] ?? false)
            ? 'Desde $' . fmt($p['precio']) . ' MXN'
            : '$' . fmt($p['precio']) . ' MXN';
        $priceClass = match($p['tipo_badge']) { 'hot' => ' orange', 'best' => ' gold', default => '' };
      ?>
      <div class="<?= $cardClass ?>"
           data-pkg="<?= $pkgLabel ?>"
           data-price="<?= htmlspecialchars($dataPrice, ENT_QUOTES) ?>"
           data-db-id="<?= (int)$p['db_id'] ?>"
           data-demo="<?= htmlspecialchars($p['demo'], ENT_QUOTES) ?>"
           data-demo-biz="<?= htmlspecialchars($p['demo_biz'], ENT_QUOTES) ?>">
        ...
        <div class="card-name"><?= htmlspecialchars($p['nombre'], ENT_QUOTES) ?></div>
        <div class="card-tagline"><?= htmlspecialchars($p['subtitulo'], ENT_QUOTES) ?></div>

        <div class="card-price">
          <?php if ($p['precio_desde'] ?? false): ?><div class="price-desde">Desde</div><?php endif; ?>
          <span class="price-currency">$</span><span class="price-amount<?= $priceClass ?>"><?= fmt($p['precio']) ?></span>
        ...
        <ul class="card-features">
          <?php foreach ($f['inc'] as $item): ?>
            <li><span class="feature-check">✓</span> <?= htmlspecialchars($item, ENT_QUOTES) ?></li>
          <?php endforeach; ?>
          <?php foreach ($f['exc'] as $item): ?>
            <li><span class="feature-cross">✗</span> <?= htmlspecialchars($item, ENT_QUOTES) ?></li>
          <?php endforeach; ?>
        </ul>
        ...
        <div class="delivery-chip"><div class="delivery-dot"></div> Entrega: <?= htmlspecialchars($p['entrega'], ENT_QUOTES) ?></div>
      </div>
      <?php endforeach; ?>
    </div>
  </div>
...
<section class="proceso section" id="proceso">
  <div class="container">
    ...
    <div class="proceso-grid">
      ...
      <div class="step-card reveal"><div class="step-num">2</div><span class="step-icon">💳</span><div class="step-title">Crea tu cuenta</div><div class="step-desc">Regístrate en segundos, tu cuenta te da acceso al panel para rastrear tu orden y hacer el pago.</div></div>
      <div class="step-card reveal"><div class="step-num">3</div><span class="step-icon">📝</span><div class="step-title">Envía tu info</div><div class="step-desc">Completa tu formulario con los datos de tu negocio, logo y lo que quieres comunicar.</div></div>
  ...
  <div class="container">
    <div class="checkout-wrap">
      ...
      <div class="checkout-info">
        ...
        <p class="section-sub" style="margin-bottom:2rem">Crea tu cuenta, elige tu paquete y en menos de 24 horas un asesor te contactará para arrancar. Todo desde tu panel personalizado.</p>
        <div style="display:flex;flex-direction:column;gap:1.25rem">
          ...
          <div class="beneficio-item"><div class="beneficio-icon-wrap">🔒</div><div><div class="beneficio-title">Pago 100% seguro</div><div class="beneficio-desc">Procesamos pagos con MercadoPago. Tus datos siempre protegidos.</div></div></div>
      ...
      <div class="checkout-form reveal">
        ...
        <div class="form-sub">Crea tu cuenta y tu primera orden queda lista al instante. Es más fácil que pedir tacos.</div>

        <!-- Alert dinámico -->
        <div class="form-alert form-alert-error" id="regAlert"></div>

        <!-- Selector de paquetes (PHP-driven) -->
        <div class="package-selector" id="pkgSelector">
          <?php
          $firstKey = array_key_first($paquetes);
          foreach ($paquetes as $key => $p):
            $label    = ($p['precio_desde'] ?? false) ? 'Desde $' . fmt($p['precio']) : '$' . fmt($p['precio']);
            $pkgId    = htmlspecialchars($p['nombre'] . ' — ' . $p['subtitulo'], ENT_QUOTES);
            $pkgPrice = ($p['precio_desde'] ?? false)
                ? 'Desde $' . fmt($p['precio']) . ' MXN'
                : '$' . fmt($p['precio']) . ' MXN';
          ?>
          <div class="pkg-option<?= $key === $firstKey ? ' active' : '' ?>"
               data-pkg="<?= $pkgId ?>"
               data-price="<?= htmlspecialchars($pkgPrice, ENT_QUOTES) ?>"
               data-db-id="<?= (int)$p['db_id'] ?>"
               onclick="selectPkg(this)">
            <?= $p['emoji'] ?> <?= htmlspecialchars($p['nombre'], ENT_QUOTES) ?> <?= $label ?>
          </div>
          <?php endforeach; ?>
        </div>

        <!-- Paquete activo display -->
        <div class="selected-pkg-display" id="selectedPkgDisplay">
          <div>
            <div style="font-size:.72rem;color:var(--color-muted);margin-bottom:2px">Paquete seleccionado</div>
            <div class="selected-pkg-name" id="selPkgName">
              <?= $paquetes[$firstKey]['emoji'] ?> <?= htmlspecialchars($paquetes[$firstKey]['nombre'] . ' — ' . $paquetes[$firstKey]['subtitulo'], ENT_QUOTES) ?>
            </div>
          </div>
          <div class="selected-pkg-price" id="selPkgPrice">$<?= fmt($paquetes[$firstKey]['precio']) ?> MXN</div>
        </div>

        <!-- Hidden: db_id del paquete activo -->
        <input type="hidden" id="fPaqueteId" value="<?= (int)$paquetes[$firstKey]['db_id'] ?>"/>

        <!-- Nombre y Apellidos -->
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Nombre *</label>
            <input type="text" class="form-input" placeholder="Juan" id="fNombre" autocomplete="given-name"/>
            <div class="field-error" id="ferr-nombre"></div>
          </div>
          <div class="form-group">
            <label class="form-label">Apellidos *</label>
            <input type="text" class="form-input" placeholder="Pérez López" id="fApellidos" autocomplete="family-name"/>
            <div class="field-error" id="ferr-apellidos"></div>
          </div>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="tel" class="form-input" placeholder="+52 55 1234 5678" id="fPhone" autocomplete="tel"/>
            <div class="field-error" id="ferr-telefono"></div>
          </div>
          <div class="form-group">
            <label class="form-label">Correo electrónico *</label>
            <input type="email" class="form-input" placeholder="tu@correo.com" id="fEmail" autocomplete="email"/>
            <div class="field-error" id="ferr-email"></div>
          </div>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="password" class="form-input" placeholder="Mínimo 8 caracteres" id="fPassword" autocomplete="new-password"/>
            <div class="field-error" id="ferr-password"></div>
          </div>
          <div class="form-group">
            <label class="form-label">Repetir contraseña *</label>
            <input type="password" class="form-input" placeholder="Repite tu contraseña" id="fPasswordR" autocomplete="new-password"/>
            <div class="field-error" id="ferr-password_confirmation"></div>
          </div>
        </div>

        <!-- Descripción -->
        <div class="form-group">
          <label class="form-label">Cuéntanos tu negocio *</label>
          <textarea class="form-textarea" placeholder="¿A qué se dedica tu negocio? ¿Qué necesitas en el sitio? ¿Tienes logo y colores?" id="fDesc"></textarea>
          <div class="field-error" id="ferr-descripcion"></div>
        </div>
        ...
        <div style="font-size:.78rem;color:var(--color-muted);margin-bottom:.5rem;font-weight:600">Métodos de pago disponibles:</div>
        ...
        <button class="btn-buy" id="regSubmitBtn" onclick="submitRegister()">🌮 ¡Crear mi cuenta y ordenar!</button>
        <div class="secure-note">🔒 Cifrado SSL · Verificación por correo · Tus datos protegidos</div>
        <p class="login-hint">¿Ya tienes cuenta? <a href="/login">Iniciar sesión →</a></p>
      </div>
    ...
    <div class="footer-grid">
      <div>
        ...
        <div class="social-links">
          <a href="https://www.facebook.com/taquerosweb" target="_blank" rel="noopener" class="social-btn sb-fb" title="Facebook" aria-label="Facebook"><svg aria-hidden="true"><use href="#ic-facebook"></use></svg></a>
          <a href="https://www.instagram.com/taquerosweb" target="_blank" rel="noopener" class="social-btn sb-ig" title="Instagram" aria-label="Instagram"><svg aria-hidden="true"><use href="#ic-instagram"></use></svg></a>
          <a href="https://www.tiktok.com/@taquerosweb" target="_blank" rel="noopener" class="social-btn sb-tk" title="TikTok" aria-label="TikTok"><svg aria-hidden="true"><use href="#ic-tiktok"></use></svg></a>
          <a href="https://www.linkedin.com/in/taquerosweb" target="_blank" rel="noopener" class="social-btn sb-in" title="LinkedIn" aria-label="LinkedIn"><svg aria-hidden="true"><use href="#ic-linkedin"></use></svg></a>
          <a href="https://wa.me/<?= $cfg['whatsapp'] ?>" target="_blank" rel="noopener" class="social-btn sb-wa" title="WhatsApp" aria-label="WhatsApp"><svg aria-hidden="true"><use href="#ic-whatsapp"></use></svg></a>
        </div>
      </div>
      <div>
      ...
      <div>
        ...
        <ul class="footer-links">
          <li><a href="mailto:<?= $cfg['email'] ?>">📧 <?= htmlspecialchars($cfg['email'], ENT_QUOTES) ?></a></li>
        ...
        <div style="margin-top:1.5rem">
          ...
          <a href="https://wa.me/<?= $cfg['whatsapp'] ?>?text=Hola%2C%20quiero%20info" target="_blank" class="btn-primary" style="display:inline-flex;padding:.7rem 1.25rem;font-size:.85rem">💬 Escríbenos</a>
        </div>
      </div>
    </div>
...
<a href="https://wa.me/<?= $cfg['whatsapp'] ?>?text=Hola%2C%20me%20interesa%20un%20sitio%20web%20de%20TaquerosWeb.com"
   class="whatsapp-float" target="_blank" rel="noopener" title="Contactar por WhatsApp" aria-label="Contactar por WhatsApp"><svg aria-hidden="true"><use href="#ic-whatsapp"></use></svg></a>
...
<div class="modal-overlay" id="successModal">
  <div class="modal-box">
    ...
    <p class="modal-msg">
      Tu orden está en la cocina 👨‍🍳<br><br>
      Te enviamos un correo de verificación. Una vez confirmado, inicia sesión en tu panel para confirmar la orden y proceder al pago.<br><br>
      <strong style="color:var(--color-lime)">Revisa tu bandeja de entrada ✉️</strong>
    </p>
    <a href="/login" class="btn-modal">Ir a iniciar sesión 🔑</a>
  </div>
</div>
  ...
  <div class="demo-modal">
    <div class="demo-modal-header">
      ...
      <div class="demo-modal-actions">
        <button class="btn-fullscreen" onclick="openDemoFullscreen()">⛶ Pantalla completa</button>
    ...
    <div class="demo-modal-footer">
      ...
      <button class="demo-thumb"
              data-demo="<?= htmlspecialchars($p['demo'], ENT_QUOTES) ?>"
              data-title="<?= htmlspecialchars($p['demo_biz'], ENT_QUOTES) ?>"
              data-pkg="<?= htmlspecialchars($p['nombre'], ENT_QUOTES) ?> — $<?= fmt($p['precio']) ?>"
              onclick="switchDemo(this)"><?= $p['emoji'] ?> <?= htmlspecialchars($p['nombre'], ENT_QUOTES) ?></button>
      <?php endforeach; ?>
...
// ─── Pricing card button → sync selector + scroll to form ────────────────────
function selectPackage(btn) {
  ...
  document.getElementById('fPaqueteId').value        = dbId;
...
function selectPkg(el) {
  ...
  document.getElementById('fPaqueteId').value        = el.dataset.dbId;
}

// ─── Registration + order submission ─────────────────────────────────────────
async function submitRegister() {
  if (document.getElementById('fWebsite').value) return; // honeypot

  // Clear previous errors
  const alertEl = document.getElementById('regAlert');
  alertEl.style.display = 'none';
  document.querySelectorAll('[id^="ferr-"]').forEach(el => el.textContent = '');
  ...
  const payload = {
    ...
    password_confirmation: document.getElementById('fPasswordR').value,
    paquete_id:            parseInt(document.getElementById('fPaqueteId').value) || 0,
  ...
  try {
    ...
    if (data.success) {
      ...
      Object.entries(data.errors).forEach(([field, msg]) => {
        const errEl  = document.getElementById('ferr-' + field);
        const inputEl = document.getElementById('f' + field.charAt(0).toUpperCase() + field.slice(1));
        if (errEl) errEl.textContent = msg;
        if (inputEl) inputEl.classList.add('input-error');
      });
      showFormAlert(data.message || 'Por favor corrige los errores indicados.');
    } else {
      showFormAlert(data.message || 'Error al registrar. Intenta de nuevo.');
    }
  } catch {
    showFormAlert('Error de conexión. Verifica tu internet e intenta de nuevo.');
  } finally {
    btn.disabled  = false;
    btn.textContent = '🌮 ¡Crear mi cuenta y ordenar!';
  }
}
...
function loadDemoInModal(url, title, pkgInfo) {
  ...
  demoIframe.src     = url;
  demoIframe.onload  = () => { demoLoader.classList.add('hidden'); demoIframe.classList.add('loaded'); };
...
function closeDemoModal() {
  ...
  document.body.style.overflow = '';
  setTimeout(() => { demoIframe.src = 'about:blank'; demoIframe.classList.remove('loaded'); demoLoader.classList.remove('hidden'); }, 400);
}
...
demoOverlay.addEventListener('click', (e) => { if (e.target === demoOverlay) closeDemoModal(); });
